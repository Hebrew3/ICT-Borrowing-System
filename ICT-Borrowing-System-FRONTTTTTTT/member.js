...
function generateReport(memberId) {
    // Simulate fetching data for the report
    const reportData = [
        { itemName: "Laptop", dateIssued: "2024-12-01", status: "Returned" },
        { itemName: "Projector", dateIssued: "2024-11-15", status: "Issued" }
    ];

    // Create and display the report
    let report = `Report for Member ID: ${memberId}\n\n`;
    report += "Item Name\tDate Issued\tStatus\n";
    reportData.forEach(item => {
        report += `${item.itemName}\t${item.dateIssued}\t${item.status}\n`;
    });

    alert(report); // Display the report in an alert (or create a PDF/file if needed)
}
